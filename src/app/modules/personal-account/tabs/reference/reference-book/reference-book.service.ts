...
export class ReferenceBookService {
  ...
  getRecords(): Observable<any[]> {
    ...
    return this.http.post<any[]>(`${environment.production}/${this.endpoint}/Filter`, this.queryData, { headers: this.getHeader() });
  }

  getDropdownData(endpoint: string): Observable<any[]>  {
    this.queryData.filters = this.queryData.filters || [];
    return this.http.post<any[]>(`${environment.production}/${endpoint}`, this.queryData, { headers: this.getHeader() });
  }

  // Получение конкретной записи по ID
  getRecord(id: string): Observable<any> {
    return this.http.get<any>(`${environment.production}/${this.endpoint}/${id}`, { headers: this.getHeader() });
  }

  // Создание новой записи
  newRecord(record: any): Observable<any> {
    return this.http.post<any>(`${environment.production}/${this.endpoint}`, record, { headers: this.getHeader() });
  }

  // Обновление существующей записи
  updateRecord(id: number, record: any): Observable<any> {
    return this.http.put<any>(`${environment.production}/${this.endpoint}/${id}`, record, { headers: this.getHeader() });
  }

  // Удаление записи
  deleteRecord(id: number): Observable<any> {
    return this.http.delete<any>(`${environment.production}/${this.endpoint}/${id}`, { headers: this.getHeader() });
  }

  getPosition() {
    return this.http.post<any>(`${environment.production}/api/Entities/Position/Filter`, { filters: [], sorts: [] }, { headers: this.getHeader() });
  }

  getPermision() {
    return this.http.post<any>(`${environment.production}/api/Entities/Permission/Filter`,
      {
        filters: [{
