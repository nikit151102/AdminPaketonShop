...
export class BarcodeManagementComponent implements OnInit, OnDestroy {
  ...
  selectProduct(product: ProductSearchResult): void {
    ...
    this.loadBarcodesForProduct(product.id);
  }
  ...
  changeBarcodePage(page: number): void {
    ...
    this.loadBarcodesForProduct(this.selectedProduct.id, page);
  }
  ...
  toggleAddForm(): void {
    ...
    if (this.showAddForm) {
      this.addBarcodeForm.reset({ 
        coefficient: 1,
        productInstanceId: this.selectedProduct?.id 
      });
      setTimeout(() => {
  ...
  addBarcode(): void {
    if (!this.selectedProduct?.id || this.addBarcodeForm.invalid) return;
  ...
  deleteBarcode(barcode: ProductBarcode): void {
    ...
    if (!confirm(`Удалить штрихкод "${barcode.barCode}"?`)) {
      return;
    }

    this.barcodeService.deleteBarcode(barcode.id).pipe(
      takeUntil(this.destroy$)
    ).subscribe({
      next: () => {
        this.showToast('Штрихкод удалён', 'success');
        this.loadBarcodesForProduct(this.selectedProduct!.id, this.currentBarcodePage);
      },
      error: (err) => {
        console.error('Ошибка удаления штрихкода:', err);
        this.showToast('Не удалось удалить штрихкод', 'error');
      }
    });
  }
